...
public class diff_match_patch {
  ...
  ////////////////////////////////////////////////////////////
  // djk20110605 -- locally modified
  // As above but use CSS classes instead of hard coding styles.
  public String diff_prettyHtmlUsingCss(LinkedList<Diff> diffs,
                                        String insertClass,
                                        String deleteClass,
                                        String equalClass) {
        StringBuilder html = new StringBuilder();
    ...
    for (Diff aDiff : diffs) {
      String text = aDiff.text.replace("&", "&amp;").replace("<", "&lt;")
          .replace(">", "&gt;").replace("\n", "&para;<br>")
          .replace("'", "&#39;").replace("\"", "&quot;"); // djk: added ' and "

      switch (aDiff.operation) {
      case INSERT:
          html.append("<ins class=\"").append(insertClass).append("\">").append(text)
              .append("</ins>");
        break;
      case DELETE:
          html.append("<del class=\"").append(deleteClass).append("\">").append(text)
              .append("</del>");
        break;
      case EQUAL:
          html.append("<span class=\"").append(equalClass).append("\">").append(text)
              .append("</span>");
        break;
      }
      if (aDiff.operation != Operation.DELETE) {
